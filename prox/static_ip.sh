...
select opt in "List VMs again" "General" "Kali" "Commando" "Caldera" "SIEM" "Logger" "Cuckoo" "Threat Pursuit" "Terminate"
do
case $opt in
	"List VMs again"
	...
	"Kali"
		...
		echo -e "Step 2. (Whitelist ProxMox)\napt list openssh-server\ndpkg-reconfigure openssh-server\n\"echo \"sshd:10.100.10.1\" >> /etc/hosts.allow\necho \"sshd: ALL >> /etc/hosts.deny\""
		echo -e "Step 3. (Enable SSH)\nsystemctl start ssh.socket"
		;;
	"Commando"
		
		;;
	"Caldera"
		echo -e "Step 1. (Add setup user)\nOpen console through ProxMox to run the following commands.\n useradd setup -M -N -P"
		;;
	"SIEM"
